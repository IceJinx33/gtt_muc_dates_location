...
# import spacy
tag2role = OrderedDict({'incident_type': "incident_type", 'incident_date': "incident_date", 'incident_location': "incident_location", 'perp_individual_id': "PerpInd", 'perp_organization_id': "PerpOrg", 'phys_tgt_id': "Target", 'hum_tgt_name': "Victim", 'incident_instrument_id': "Weapon"})
incident_type_dict = {}
...
def read_files(div):
    ...
    with open(doc_file) as f_doc:
        for line in f_doc:
            ...
            doc_text = line_json["dateline"] + " " + line_json["text"]
                ...
                for keyval in content[1:]:
                    ...
                    if key == tag:
                        if tag == "incident_type":
                            ...
                            continue
                        if value:
                            if tag == "incident_location" or tag == "incident_date":
                                candidate = []
                                for value_key in value:
                                    loc_lst = [] 
                                    for value_str in value[value_key]:
                                        if tag == "incident_location":
                                            if '(' in value_str:
                                                value_str = " ".join(value_str.split()[:-1])
                                        loc_lst.append(value_str.lower())
                                    candidate.append(loc_lst)
                                template[role] = candidate
                            else:
                                candidate = []
                ...
                for entity in template[role]:
                    ...
                    for mention in entity:
                        if mention in doc_text_no_n:
                            ...
                            # added
                            # if role == "incident_location" or role == "incident_date":
                                # mention_idxs.append([mention, -1])
                            problematic_mention_docids.append([mention, docid])
                    mention_idxs_sorted = sorted(mention_idxs, key=lambda m_idx: m_idx[1]) # rank mentions by mention's first appearence
                    # if mention_idxs != mention_idxs_sorted: import ipdb; ipdb.set_trace()
                    if mention_idxs_sorted:
