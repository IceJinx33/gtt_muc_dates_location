...
role_list = ["incident_type", "incident_date", "incident_location", "PerpInd", "PerpOrg", "Target", "Victim", "Weapon"]
                        ...
                        if temp_token_id == self.SEP:
                            ...
                            for position in position_buf:
                                ...
                                if len(s_e_pair) == 2:
                                    ...
                                    if extract_tokens:
                                        if len(extract_tokens) <= 25: 
                                            candidate_str = " ".join(extract_tokens).replace(" ##", "").replace(" - ", "-")
                                            if sep_cnt != 7 or "bomb" not in candidate_str:
...
if __name__ == "__main__":
    ...
    if args.do_predict:
        ...
        print(checkpoints)
        model = NERTransformer.load_from_checkpoint(checkpoints[-1]).to(get_device())
        if args.debug:
